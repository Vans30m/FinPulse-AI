...
export default function AISummaryCard({
  ...
  const getRecColor = (rec: string) => {
    ...
    if (r.includes("BUY")) return "text-emerald-600 dark:text-emerald-450 bg-emerald-500/10 border-emerald-500/20";
    if (r.includes("SELL")) return "text-rose-500 bg-rose-500/10 border-rose-500/20";
    return "text-amber-500 bg-amber-500/10 border-amber-500/20";
  };

  const generatedSummary = summary || `AI Analysis indicates that ${symbol} is showing solid technical support near ${support || "pivots"}. With strong ${momentum} and a ${trend.toLowerCase()} trend pattern, the asset represents a balanced risk/reward trade setup at current valuations. Momentum gauges suggest a short-term resistance checkpoint near ${resistance || "highs"}.`;
      ...
      <div className="grid grid-cols-2 gap-4 relative z-10">
        ...
        <div className="p-3 bg-slate-50 dark:bg-white/[0.01] rounded-xl border border-slate-100 dark:border-white/5 space-y-1">
          ...
          <div className="text-sm font-black text-slate-850 dark:text-white font-mono">
            {support ? typeof support === "number" ? `$${support.toFixed(2)}` : support : "N/A"}
          </div>
        </div>
        ...
        <div className="p-3 bg-slate-50 dark:bg-white/[0.01] rounded-xl border border-slate-100 dark:border-white/5 space-y-1">
          ...
          <div className="text-sm font-black text-slate-850 dark:text-white font-mono">
            {resistance ? typeof resistance === "number" ? `$${resistance.toFixed(2)}` : resistance : "N/A"}
          </div>
        </div>
