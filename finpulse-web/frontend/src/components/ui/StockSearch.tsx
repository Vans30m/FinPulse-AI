...
function resolveSymbolType(symbol: string): "Stocks" | "Indices" | "Crypto" | "Forex" | "Commodities" {
  ...
  if (upper.startsWith("^")) return "Indices";
  return "Stocks";
}
...
export default function StockSearch({
    ...
    const delayDebounceFn = setTimeout(async () => {
      try {
        ...
        const enriched = (data || [])
          ...
          .map((item: any) => ({
            ...item,
            type: item.type || resolveSymbolType(item.symbol)
          }));
    ...
    if (simple) {
      ...
      if (onlyTypes.length > 0) {
        filteredResults = filteredResults.filter(item => {
          ...
          return onlyTypes.includes(category);
        });
      }
      return { All: filteredResults };
    }

    let listToProcess = query.trim().length === 0 ? recentSearches : results;
    
    if (excludeTypes.length > 0) {
      listToProcess = listToProcess.filter(item => {
        const category = resolveSymbolType(item.symbol);
        return !excludeTypes.includes(category);
      });
    }
  ...
  }, [results, activeFilter, query, recentSearches, excludeTypes, onlyTypes, simple]);
  ...
  const filterTabs = useMemo(() => {
    if (simple) return [];
    const defaultTabs = ["All", "Stocks", "Indices", "Crypto", "Forex", "Commodities"];
    if (excludeTypes.length > 0) {
      return defaultTabs.filter(tab => tab === "All" || !excludeTypes.includes(tab));
    }
    return defaultTabs;
  }, [excludeTypes, simple]);
  ...
  const navItems = useMemo(() => {
    let rawItems = query.trim().length === 0 ? recentSearches : Object.values(groupedAndFilteredResults).flat();
    if (simple) {
      rawItems = results;
    }
    if (excludeTypes.length > 0) {
    ...
    return rawItems;
  }, [query, recentSearches, groupedAndFilteredResults, excludeTypes, onlyTypes, simple, results]);
