      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 z-10">
        ...
        <div className="lg:col-span-5 flex flex-col justify-between space-y-4">
          <div className="flex flex-col space-y-3">
            ...
            >
              ...
              <span className="text-sm text-slate-500 dark:text-slate-400 truncate max-w-[200px] font-medium group-hover:text-slate-700 dark:group-hover:text-slate-350 transition-colors">
                {brief.company}
              </span>
            </a>

            <p className="text-slate-600 dark:text-slate-300 font-semibold text-xs leading-normal">
              {brief.summary}
            </p>
          </div>
        ...
        <div className="lg:col-span-7 flex flex-col justify-between space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Target & Stoploss */}
            <div className="bg-slate-50/50 dark:bg-slate-950/30 p-3 rounded-2xl border border-slate-200/60 dark:border-slate-900/60 grid grid-cols-2 gap-2">
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Target</span>
                <p className="text-lg font-black text-emerald-600 dark:text-emerald-450 mt-1">{cSymbol}{brief.target.toFixed(2)}</p>
              </div>
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Stop Loss</span>
                <p className="text-lg font-black text-rose-600 dark:text-rose-455 mt-1">{cSymbol}{brief.stopLoss.toFixed(2)}</p>
              </div>
            </div>

            {/* Risk & Holding Profile */}
            <div className="bg-slate-550/20 dark:bg-white/[0.02] p-3 rounded-2xl border border-slate-200/50 dark:border-white/[0.04] grid grid-cols-2 gap-2">
              <div className="min-w-0">
                <span className="text-[9px] uppercase font-bold text-slate-500 dark:text-slate-400 block tracking-wider">Holding</span>
                <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block truncate mt-1">{brief.holdingPeriod}</span>
              </div>
              <div className="min-w-0">
                <span className="text-[9px] uppercase font-bold text-slate-500 dark:text-slate-400 block tracking-wider">Risk Profile</span>
                <span className={`text-[10px] px-2 py-0.5 rounded-lg font-black uppercase leading-tight mt-1 inline-block ${getRiskColors(brief.risk)}`}>
                  {brief.risk}
                </span>
              </div>
            </div>
