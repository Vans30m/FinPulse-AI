...
const EarningsCard = memo(function EarningsCard({
  ...
  return (
    <motion.div
      ...
      className="glass-card flex flex-col justify-between p-3.5 sm:p-5 cursor-pointer relative overflow-hidden group hover:border-blue-400/40 dark:hover:border-cyan-500/30 hover:shadow-lg dark:hover:shadow-glow transition-all duration-300 h-full"
    >
      ...
      <div>
        <div className="flex items-start justify-between gap-1.5 mb-3.5">
          <div className="flex items-center gap-2.5 min-w-0">
            <StockLogo symbol={earning.symbol} name={earning.name} className="h-9 w-9 sm:h-11 sm:w-11 shrink-0" imgSizeClass="w-6 h-6 sm:w-8 sm:h-8" />
        ...
        <div className="grid grid-cols-2 gap-2.5 border-t border-slate-100 dark:border-white/5 pt-2.5 pb-2.5 mb-2.5">
          <div>
            <span className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Price ({earning.currency})</span>
            <div className="flex flex-wrap items-baseline gap-x-1 mt-0.5">
              <span className="font-mono text-sm sm:text-lg font-black tracking-tight text-slate-800 dark:text-slate-100 leading-none">
                {earning.price !== undefined ? `${earning.price.toFixed(2)}` : "N/A"}
              </span>
              <span className={`inline-flex items-center gap-0.5 text-[9px] sm:text-xs font-bold font-mono ${isPositive ? 'text-emerald-650 dark:text-emerald-400' : 'text-rose-500'}`}>
                {isPositive ? <TrendingUp className="h-2 w-2 sm:h-2.5 sm:w-2.5" /> : <TrendingDown className="h-2 w-2 sm:h-2.5 sm:w-2.5" />}
                {isPositive ? "+" : ""}{earning.changePercent ? earning.changePercent.toFixed(2) : "0.00"}%
              </span>
            </div>
          </div>
          <div>
            <span className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Mkt Cap</span>
            <span className="font-mono text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200 mt-1 block truncate">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-t border-slate-100 dark:border-white/5 pt-2.5 mt-auto">
        <div className="flex items-center gap-1 text-slate-500 dark:text-slate-400">
          <Calendar className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-slate-400" />
          <div className="flex flex-col">
            <span className="text-[7.5px] font-bold text-slate-400 uppercase tracking-wider">Earnings Date</span>
            <span className="font-mono text-[10px] sm:text-xs font-semibold text-slate-700 dark:text-slate-300">
        ...
        <div className="text-left sm:text-right">
          <span className="text-[7.5px] font-bold text-slate-400 uppercase tracking-wider block">Est. EPS</span>
          <span className="font-mono text-[10px] sm:text-xs font-black text-slate-800 dark:text-slate-200 mt-0.5 block">
...
function EarningsGrid({
  ...
  if (isLoading) {
    return (
      <div className="flex overflow-x-auto snap-x snap-mandatory scrollbar-none gap-2.5 sm:gap-6 pb-4 w-full">
        {Array.from({ length: 10 }).map((_, idx) => (
          <div
            key={idx}
            className="w-[calc(50%-5px)] sm:w-[calc(50%-12px)] lg:w-[calc(33.33%-16px)] flex-shrink-0 snap-start glass-card flex flex-col justify-between p-3.5 sm:p-5 border border-slate-200/20 dark:border-white/5 animate-pulse min-h-[220px]"
          >
            <div>
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3 w-full">
                  <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-slate-200 dark:bg-night-800 shrink-0" />
                  <div className="space-y-1.5 w-1/2">
                    <div className="h-3 w-12 bg-slate-200 dark:bg-night-800 rounded" />
                    <div className="h-2 w-16 bg-slate-200 dark:bg-night-800 rounded" />
                  </div>
                </div>
  ...
  return (
    <div className="relative w-full group">
      ...
      <div
        ref={scrollRef}
        className="flex overflow-x-auto snap-x snap-mandatory scrollbar-none gap-2.5 sm:gap-6 pb-4 w-full"
      >
        {earnings.map((earning) => (
          <div
            key={earning.symbol}
            className="w-[calc(50%-5px)] sm:w-[calc(50%-12px)] lg:w-[calc(33.33%-16px)] flex-shrink-0 snap-start"
          >
            <EarningsCard
              earning={earning}
              onClick={onCardClick}
...
const CountryTabs = memo(function CountryTabs({
  ...
  return (
    <div className="w-full overflow-x-auto scrollbar-none border-b border-slate-200/50 dark:border-white/5 pb-2">
      <div className="flex gap-1.5 min-w-max p-1 bg-slate-100/50 dark:bg-night-950/40 rounded-2xl border border-slate-200/40 dark:border-white/5 w-fit">
        {COUNTRIES.map((country) => {
          ...
          return (
            <button
              ...
              className={`
                relative flex items-center gap-1.5 px-3.5 py-1.5 sm:px-5 sm:py-2.5 rounded-xl font-display text-xs sm:text-sm font-semibold transition-all duration-300 select-none z-10
                ${isActive
            ...
            >
              ...
              <span className="text-xs sm:text-base">{country.flag}</span>
...
export default function GlobalEarningsCalendar() {
  ...
  return (
    <div className="w-full space-y-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-200/50 dark:border-white/5 pb-3">
        <div className="flex items-center gap-3">
          <div className="p-2 sm:p-3.5 rounded-2xl bg-gradient-to-tr from-blue-600 to-indigo-600 dark:from-cyan-500/10 dark:to-cyan-400/20 text-white dark:text-cyan-400 border border-indigo-500/10 dark:border-cyan-500/30 shadow-md">
            <Sparkles className="h-4.5 w-4.5 sm:h-6 sm:w-6 animate-pulse" />
          </div>
          <div>
            <span className="text-[9px] sm:text-[10px] bg-blue-50 dark:bg-cyan-500/10 text-blue-600 dark:text-cyan-400 border border-blue-200/40 dark:border-cyan-500/20 px-1.5 py-0.5 rounded font-mono uppercase tracking-widest font-black">
              Upcoming Schedule
            </span>
            <h2 className="text-base sm:text-2xl font-bold text-slate-900 dark:text-white mt-1">
