...
export default function StockScreener() {
  ...
  const handleSelectStock = async (symbol: string) => {
    ...
    try {
      ...
      const cachedDetailsStr = localStorage.getItem(`screener-asset-details-${symbol}`);
      let cachedDetails: any = null;
      if (cachedDetailsStr) {
        try {
          const parsed = JSON.parse(cachedDetailsStr);
          if (Date.now() - parsed.timestamp < 4 * 60 * 60 * 1000) {
            cachedDetails = parsed.data;
          }
        } catch (e) {}
      }

      if (cachedDetails) {
        setAssetDetails(cachedDetails);
      }
      ...
      // 2. Fetch slower data (AI score & news & unified asset details) asynchronously in the background
      Promise.all([
        getAIScore(symbol).catch(() => ({ score: 70 })),
        getCompanyNews(symbol).catch(() => []),
        cachedDetails ? Promise.resolve(cachedDetails) : getUnifiedAssetDetails(symbol).catch(() => null)
      ]).then(([_aiScore, newsData, assetDetailsData]) => {
        setCompanyNews(newsData);
        if (assetDetailsData) {
          setAssetDetails(assetDetailsData);
          localStorage.setItem(`screener-asset-details-${symbol}`, JSON.stringify({
            timestamp: Date.now(),
            ...
            <div className="flex-1 min-w-0 space-y-6 w-full">
              ...
              <div id="screener-market-data" className="scroll-mt-28 border-b border-slate-200/60 dark:border-white/10 pb-4 mt-6">
                ...
                {isMarketDataOpen && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mt-4 animate-fadeIn">
                    {[
                      { label: 'Current Price', value: `${currencySymbol}${(assetDetails?.statistics?.price ?? selectedStock.price).toFixed(2)}`, desc: 'Latest trading price' },
                      { label: 'Price Change', value: `${currencySymbol}${(assetDetails?.statistics?.change ?? (selectedStock.price * selectedStock.changePercent / 100)).toFixed(2)}`, desc: 'Absolute price movement' },
                      { label: 'Change %', value: `${(assetDetails?.statistics?.changePercent ?? selectedStock.changePercent).toFixed(2)}%`, desc: 'Percentage price movement' },
                      { label: 'Open', value: `${currencySymbol}${(assetDetails?.statistics?.open ?? (selectedStock.price * 0.99)).toFixed(2)}`, desc: 'Today\'s opening price' },
                      { label: 'High', value: `${currencySymbol}${(assetDetails?.statistics?.dayHigh ?? selectedStock.high52w * 0.95).toFixed(2)}`, desc: 'Today\'s highest price' },
                      { label: 'Low', value: `${currencySymbol}${(assetDetails?.statistics?.dayLow ?? selectedStock.low52w * 1.05).toFixed(2)}`, desc: 'Today\'s lowest price' },
                      { label: 'Previous Close', value: `${currencySymbol}${(assetDetails?.statistics?.previousClose ?? (selectedStock.price * 1.01)).toFixed(2)}`, desc: 'Yesterday\'s closing price' },
                      { label: 'Volume', value: (assetDetails?.statistics?.volume ?? 5000000).toLocaleString(), desc: 'Shares traded today' },
                      { label: 'Average Volume', value: (assetDetails?.statistics?.averageVolume ?? 6000000).toLocaleString(), desc: '3-month average volume' },
              ...
              <div id="screener-valuation" className="scroll-mt-28 border-b border-slate-200/60 dark:border-white/10 pb-4 mt-6">
                ...
                {isValuationOpen && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mt-4 animate-fadeIn">
                    {[
                      { label: 'Market Capitalization', value: `${currencySymbol}${selectedStock.marketCap.toLocaleString(undefined, { maximumFractionDigits: 0 })} ${isIndian ? 'Cr.' : 'M'}`, desc: 'Total market value of equity' },
                      { label: 'Enterprise Value', value: `${currencySymbol}${(assetDetails?.statistics?.enterpriseValue ? (isIndian ? assetDetails.statistics.enterpriseValue / 10000000 : assetDetails.statistics.enterpriseValue / 1000000) : (selectedStock.marketCap * 1.05)).toLocaleString(undefined, { maximumFractionDigits: 0 })} ${isIndian ? 'Cr.' : 'M'}`, desc: 'Total business value including net debt' },
                      { label: 'P/E Ratio', value: `${selectedStock.peRatio.toFixed(1)}x`, desc: 'Price-to-earnings multiple' },
                      { label: 'Forward P/E', value: `${(assetDetails?.statistics?.forwardPe ?? (selectedStock.peRatio * 0.85)).toFixed(1)}x`, desc: 'Expected forward P/E multiple' },
                      { label: 'Price-to-Book', value: `${(selectedStock.price / selectedStock.bookValue).toFixed(2)}x`, desc: 'Price to book value multiple' },
                      { label: 'Price-to-Sales', value: `${(assetDetails?.statistics?.priceToSales ?? 4.25).toFixed(2)}x`, desc: 'Price to sales turnover multiple' },
                      { label: 'EV / EBITDA', value: `${(assetDetails?.statistics?.evToEbitda ?? 12.4).toFixed(1)}x`, desc: 'Enterprise value to EBITDA multiple' },
                      { label: 'EV / Revenue', value: `${(assetDetails?.statistics?.evToRevenue ?? 3.5).toFixed(1)}x`, desc: 'Enterprise value to sales multiple' },
                    ].map((item) => (
                      <div key={item.label} className="bg-slate-50 dark:bg-white/[0.01] hover:bg-slate-100/50 dark:hover:bg-white/[0.02] border border-slate-150/40 dark:border-white/[0.03] p-3 sm:p-4 rounded-2xl flex items-start gap-2 sm:gap-3 transition-all">
                        <div className="flex flex-col min-w-0">
                          <span className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider truncate">{item.label}</span>
              ...
              <div id="screener-fundamentals" className="scroll-mt-28 border-b border-slate-200/60 dark:border-white/10 pb-4 mt-6">
                ...
                >
                  <h3 className="text-sm font-extrabold text-slate-800 dark:text-white tracking-tight flex items-center gap-2">
                    <span className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-cyan-400 border border-emerald-500/10">
              ...
              <div id="screener-shareholding" className="scroll-mt-28 border-b border-slate-200/60 dark:border-white/10 pb-8 mt-6 w-full relative overflow-hidden">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-white/5 pb-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 font-bold mt-1">Numbers in percentages (Yearly)</p>
                  </div>
                </div>
                    ...
                    {[
                      ...
                      <div key={item.label} className="bg-slate-50/50 dark:bg-white/[0.01] border border-slate-200/50 dark:border-white/5 p-3 sm:p-4 rounded-2xl flex flex-col gap-1.5 sm:gap-2">
                        <div className="flex justify-between items-center text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-400">
                          ...
                          <span className="font-mono text-slate-950 dark:text-white font-black text-xs sm:text-sm">{item.value.toFixed(2)}%</span>
                        </div>
                  ...
                  <table className="w-full text-left border-collapse table-fixed text-[9px] md:text-[10px] xl:text-xs min-w-[900px]">
                    <thead>
                      <tr className="border-b border-slate-200/50 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.01] font-black uppercase text-slate-455 tracking-wider">
                        ...
                        {shareholdingData?.quarters.filter((_, i) => i % 4 === 3)?.map((q) => (
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-white/[0.02] font-mono font-bold text-slate-900 dark:text-slate-100">
                      {shareholdingData?.rows.map((row) => (
                        <tr key={row.label} className="hover:bg-slate-50/70 dark:hover:bg-white/[0.02] transition-colors">
                          <td className="py-3 px-3.5 text-left font-sans font-extrabold text-slate-955 dark:text-white">
                          ...
                          {row.values.filter((_, i) => i % 4 === 3).map((v, i) => (
