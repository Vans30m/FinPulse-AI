...
profileRoutes.put('/', protect, async (req, res) => {
    try {
        const user = await getOrCreateUser(req);
        const { name, username, phone, country, timezone, currency, bio, occupation, avatar, riskProfile, investmentGoal, investmentHorizon, experienceLevel, preferredExchange, baseCurrency, taxCountry, connectedAccounts } = req.body;
